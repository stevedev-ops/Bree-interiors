...
const Home = () => {
    ...
    useEffect(() => {
        const fetchContent = async () => {
            try {
                ...
                if (projectsData) {
                    // Filter featured and published projects
                    setFeaturedProjects(projectsData.filter(p => p.published && p.featured).slice(0, 6));
                }
            } catch (err) {
    ...
    return (
        <PageTransition>
            <SEO
                title="Bree Interiors | Luxury Afro-Modern Design"
                description="Luxury Afro-Modern interiors that blend contemporary global design with authentic Kenyan heritage."
            />
            <div ref={pageRef}>
                {/* Hero Section */}
                <section
                        ...
                        <div className="grid-2-col">
                            ...
                            <div className="reveal delay-200" style={{ position: 'relative' }}>
                                <div style={{ aspectRatio: '4/5', backgroundColor: 'var(--color-sand)', overflow: 'hidden' }}>
                                    <ImageWithSkeleton src="/about_studio.png" alt="Bree Interiors Studio Workspace" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                </div>
                            </div>
                        </div>
                    </div>
